...
## BK-Plot Generation Functions

## returns slope of a line given x, and y
## x should be a vector of 2 points, same with y
slope <- function(x, y){
  return(cov(x, y)/var(x))
}

## Finds intercept given slope, x, and y vectors
intercept <- function(x, y, m){
  b <- mean(y) - (m * mean(x))
  return(b)
}

## predict for y given an x value, slope, and intercept
y_solver <- function(x, m, b){
  return(m*x+b)
}

## Gets the data to form BK plots
## Parameters: 
##    data, 
##    var_index: response variable index,
##    by_index: confounding variable index,
## Returns: List with two data frames
##    [[1]]: data frame of the proportions by explanatory vars
##    [[2]]: data frame of the proportions in aggregate

getBKPlotData <- function(data = df, var_index = 3, by_index = 1){
  ## Explanatory_vars
  # data = moderate_df
  # var_index = 3
  # by_index = 1
  ...
  ## get the column names of explanatory vars
  col_names <- names(data)[grouping_cols]
  ...
  all_combos <- expand.grid(list_cols)
  ...
  temp <- data |>
    ...
    suppressMessages() |>
    replace_na(list(n = 0)) 
  ## Get counts of every combination of categories, response + explanatory
  
  ## Get the name of the response variable
  col_resp = names(data)[var_index]
  
  ## Takes arbitrary value for response, as the complement is the other value
  resp <- data[[var_index]][1]
  
  ## this is the column number that is not the response or confounding one
  ## notice that is not a "free" variable as it has to be this one given 3 columns
  idx <- c(1:ncol(data))[-c(var_index, by_index)]
  
  ## the column name of that variable
  target_name <- names(data)[idx]
  
  ## the data frame that contains the marginal proportions )
  prop_df <- data |>
    ...
    right_join(temp) |>
    suppressMessages() |>
    select(names(data)[1:3], n, total) |>
    mutate(
      prop = n/total * 100
    ) |>
    ## using rlang, basically filtering for only responses where the response column
    ## is only our single response value we found
    filter(!!sym(col_resp) == !!(resp)) |>
    select(!!(col_resp), everything(), -n, -total) |>
    ## creates a wide data given our explanatory var
    pivot_wider(
      names_from = !!(target_name),
      values_from = prop
    )
  
  ## will always be the third column of this data frame, as 
  ## the fourth column will just be the prop of the "other" response
  by_cat <- names(prop_df)[3]
  by_totals <- data |>
    group_by(across(all_of(by_index))) |>
    count() |> 
    rename(total = n)
  
  ## the column var name that is not response or underlying
  col_name <- names(data)[idx]
  ## calculates the aggregate proportion of response by just sole explanatory var
  actual_prop <- data |>
    group_by(across(all_of(grouping_cols))) |>
    count() |>
    filter(!!sym(target_name) == !!(by_cat)) |>
    right_join(by_totals) |>
    suppressMessages() |>
    mutate(
      prop = n/total * 100
    ) |>
  ...
  return(list(prop_df, actual_prop))
}

## default color wheel from GGPlots 
gg_color_hue <- function(n) {
  hues = seq(15, 375, length = n + 1)
  hcl(h = hues, l = 65, c = 100)[1:n]
}

## Given data frame, response var, and confounding var,
## returns plots
createBKPlot <- function(data = df, var_index = 3, by_index = 1){
  ## Gets BK Plot data
  listy <- getBKPlotData(data, var_index, by_index)
  
  ## Column of confounding var
  by_name <- names(data)[by_index]
  
  ## Overall Data, sorting it to make sure it matches up with plot points
  plotData <- listy[[1]] |>
    arrange(!!sym(by_name))
  
  ## data for specific points to plot
  plotPoints <- listy[[2]] |>
    arrange(!!sym(by_name))
  
  ## just to show n categories in the by category
  n <- data[[by_index]] |>
    levels() |>
    length()
  ...
  ## by category in plotPoints because always first one due to previous work
  by_cat <- plotPoints[[1]][1]
  idx <- which(names(plotData) == by_cat)
  ...
  x_axis = c(rep(0, n),rep(100, n))
  ## other idx first because when 0% of our index, that means all the other category
  y_axis = c(plotData[[other_idx]], plotData[[idx]])
  
  colors = gg_color_hue(n)
  
  ## Plot of just the points
  plot(x_axis,y_axis, ylim = c(0,110),
       xlab= paste0(by_cat, " % of group"), ylab= paste("%", resp))
  
  ## Drawing the lines between the points
  for(i in 1:n){
    temp_x <- c(x_axis[i],x_axis[i+n])
    temp_y <- c(y_axis[i],y_axis[i+n])
    
    my_slope = slope(temp_x, temp_y)
    my_int = intercept(temp_x, temp_y, my_slope)
    
    ## draw the line
    abline(my_int, my_slope, col = colors[i])
    
    ## plot the aggregate points
    new_x = plotPoints$prop[i]
    #print(paste("x", new_x))
    new_y = y_solver(new_x, my_slope, my_int)
    #print(paste("y", new_y))
    points(new_x, new_y, col = colors[i], cex = 1.5, pch = 16)
    text_strings <- format(c(new_x, new_y), digits = 4)
    text(new_x, new_y, labels = paste0("(", text_strings[1], ", ",
                                       text_strings[2], ")"),
         cex = 0.5, pos = 3)
    
  }
  ## drawing a horizontal line between the last y point plotted as some potential
  ## illusions occured otherwise
  #abline(h = new_y, col = "purple", lty = "dotted")
  #title(main="B-K Plot")
  # 
  col_vec <- c()
  for(i in 1:n){
  ...
  legend(0, 112, legend= plotData[[names(data)[by_index]]],
